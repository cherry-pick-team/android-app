...
export default function(state = getInitialState(), action) {
	switch (action.type) {
		...
		case SEARCH_NEXT: {
			const currentSong = state.nextSong ? state.nextSong : state.currentSong;
			const nextSong = currentSong < state.entries.length ? currentSong + 1 : null;
			const prevSong = currentSong > 0 ? currentSong - 1 : null;
		...
		case SEARCH_PREV: {
			const currentSong = state.prevSong ? state.prevSong : state.currentSong;
			const nextSong = currentSong < state.entries.length ? currentSong + 1 : null;
			const prevSong = currentSong > 0 ? currentSong - 1 : null;
