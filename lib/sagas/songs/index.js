...
import {play, pause} from '../../services/streaming';
...
function* handleStatus({payload}) {
	...
	if (payload.type === 'music_status') {
		const songsIdFiltered = yield select((state) => {
			return Object.entries(state.songs).filter(([id, song]) => {
				return typeof song.playingChunk === 'number';
			})[0];
		});

		if (songsIdFiltered && songsIdFiltered.length > 0) {
			yield put(setProgress(songsIdFiltered[0], payload.data.position / payload.data.duration));
		}
	}
}
