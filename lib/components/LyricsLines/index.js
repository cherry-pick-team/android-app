...
import {View, Text, ScrollView, TouchableNativeFeedback, Timers} from 'react-native';
...
export default class LyricsLines extends Component {
	...
	renderLineWithFeedback(start, end, index) {
		return (
			...
			<ScrollView style={styles.container}>
				...
				{this.props.song.chunks.map(([start, end], index) => {
					return this.state.currentChunk === index ?
						this.renderLine(start, end, index) :
						this.renderLineWithFeedback(start, end, index);
				})}
			</ScrollView>
		)
	}
