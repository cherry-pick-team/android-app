...
export default class ProgressLine extends Component {
	...
	_handleRelease(event, gesture) {
		...
		if (progress >= 0 && progress <= 1) {
			// this.props.setProgress(this.state.progress + gesture.dx / this._getWidth(), this.props.song.song.id);
		}
	...
	_drawCircle() {
		const radius = this.state.isActive ? 12 : 6;
	...
	_drawAccentPart(start, stop) {
		const duration = this.props.song.song.duration;
