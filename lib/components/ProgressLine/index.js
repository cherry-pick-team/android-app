...
export default class ProgressLine extends Component {
	static propTypes = {
		...
		songId: PropTypes.object.isRequired,
	...
	_handleRelease(event, gesture) {
		...
		if (progress >= 0 && progress <= 1) {
			this.props.seek((this.state.progress + gesture.dx / this._getWidth()) * this.props.duration);
		}
	...
	_drawTimeParts() {
		return this.props.chunks.map((chunk) => this._drawAccentPart(chunk[0], chunk[1]));
	}
