...
export function getStreamURL(mongoId, from, to) {
	return `https://zsong.ru/crop/get_song/?id=${mongoId}&from_ms=${from}&to_ms=${to}`;
}
...
export function subscribe(listener) {
	...
	return () => {
		const listenerId = listeners.indexOf(listener);
		if (listenerId !== -1) {
...
function runStatusLoop() {
	const listener = DeviceEventEmitter.addListener(
		'AudioBridgeEvent', (evt) => {
			listeners.forEach((listener) => listener(evt, currentId));
		}
	)
}
